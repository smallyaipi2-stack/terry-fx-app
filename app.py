...
with tab_dashboard:
    ...
    with col_main:
        ...
        with cl:
            st.subheader("🔄 快速換算")
            amt = st.number_input("金額", min_value=0.0, value=100.0, key="fx_a")
            fc = st.selectbox("從", list(rates_dict.keys()), index=1, key="fx_f")
            tc = st.selectbox("到", list(rates_dict.keys()), index=0, key="fx_t")
            if st.button("計算", use_container_width=True): st.success(f"### {(amt * rates_dict[fc]) / rates_dict[tc]:,.2f} {tc}")
        with cr:
            st.subheader("📈 歷史趨勢")
        ...
        with st.expander("🚀 海外佈局預警", expanded=True):
            ...
            with ti:
                c1, c2, c3 = st.columns(3); ic = c1.selectbox("幣別", [n for n in rates_dict.keys() if n != '台幣 (TWD)'], index=4, key="im_c"); ib = c2.number_input("基準", value=7.10, format="%.4f", key="im_b"); ia = c3.number_input("金額", value=1000000, key="im_a")
                imp = ia * (rates_dict[ic] - ib)
                if imp > 0: st.error(f"⚠️ 支出預計增加 {imp:,.0f} 元")
                elif imp < 0: st.success(f"✅ 支出預計節省 {abs(imp):,.0f} 元")
            with te:
                c1, c2, c3 = st.columns(3); ec = c1.selectbox("幣別", [n for n in rates_dict.keys() if n != '台幣 (TWD)'], index=6, key="ex_c"); eb = c2.number_input("預算", value=24.00, format="%.4f", key="ex_b"); ea = c3.number_input("收款", value=500000, key="ex_a")
        ...
        if stocks_dict:
            ks = list(stocks_dict.keys()); s1 = st.columns(4)
            for i in range(4): n = ks[i]; p, c = stocks_dict[n]; s1[i].metric(n, f"{p:.2f}", f"{c:+.2f}")
            s2 = st.columns(4)
            for i in range(4, 8): n = ks[i]; p, c = stocks_dict[n]; s2[i-4].metric(n, f"{p:.2f}", f"{c:+.2f}")
        st.divider()
        st.subheader("📋 多幣別對照矩陣 (Cross Rates)")
        if rates_dict:
            mc = list(rates_dict.keys()); md = [[round(rates_dict[r] / rates_dict[c], 4) for c in mc] for r in mc]
    ...
    with col_right:
        ...
        st.metric("目前累計營收", f"{total_actual_revenue:,.0f}")
        # 時間進度計算
...
with tab_revenue:
    st.header("📅 年度業績與達成率分析")
    st.write("修改下方數據後，請務必點擊右側的『儲存數據』以永久記憶。")
    
    col_t1, col_t2 = st.columns([3, 1])
    with col_t1:
        # 顯示編輯表格
        edited_df = st.data_editor(st.session_state.revenue_data, use_container_width=True, hide_index=True, height=475)
    ...
    with c_c1:
        ...
        chart = alt.Chart(chart_data).mark_bar().encode(
            x=alt.X('月份:N'), y=alt.Y('金額:Q', axis=alt.Axis(format=',.0f')),
