...
def load_data():
    ...
    # 初始預設數據 (目標 1.4 億平均分配) [cite: 2026-01-20]
    return pd.DataFrame({
...
with tab1:
    ...
    with col_l:
        ...
        with cl:
            st.subheader("🔄 快速換算")
            a = st.number_input("試算金額", min_value=0.0, value=100.0, key="fxa")
            f = st.selectbox("從", list(rates_dict.keys()), index=1, key="fxf")
            t = st.selectbox("到", list(rates_dict.keys()), index=0, key="fxt")
            if st.button("立即計算", use_container_width=True):
                st.success(f"### {(a * rates_dict[f]) / rates_dict[t]:,.2f} {t}")
        with cr:
            st.subheader("📈 歷史趨勢")
            hc = st.selectbox("幣別", [n for n in rates_dict.keys() if n != '台幣 (TWD)'], key="hc")
        ...
        with st.expander("🚀 海外佈局預警", expanded=True):
            ...
            with t_ex:
                ...
                if imp_e > 0: st.success(f"✅ 收益增加 {imp_e:,.0f} 元")
                elif imp_e < 0: st.error(f"⚠️ 收益縮水 {abs(imp_e):,.0f} 元")
    ...
    with col_r:
        ...
        current_sum = st.session_state.revenue_data["實際營收 (TWD)"].sum()
        st.metric("目前累計營收 (TWD)", f"{current_sum:,.0f}")
        ...
        act_p = min(current_sum / TARGET_TOTAL, 1.0)
        st.progress(act_p)
...
with tab2:
    ...
    with c_edit:
        ...
        # *** 關鍵修復：立即計算達成率 ***
        edited_df["達成率 (%)"] = (edited_df["實際營收 (TWD)"] / edited_df["業績目標 (TWD)"] * 100).fillna(0)
    ...
    with c_save:
        if st.button("💾 儲存並同步數據", use_container_width=True):
            ...
            st.rerun()
        st.divider()
        sum_a = edited_df["實際營收 (TWD)"].sum()
        st.metric("年度總實績", f"{sum_a:,.0f}")
        st.metric("年度總達成率", f"{(sum_a/TARGET_TOTAL):.2%}")
