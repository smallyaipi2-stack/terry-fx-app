...
with tab1:
    ...
    with col_l:
        ...
        if stocks_dict:
            ...
            s_row2 = st.columns(4)
            for i in range(4, 8): nk = k_list[i]; pk, ck = stocks_dict[nk]; s_row2[i-4].metric(nk, f"{pk:,.2f}", f"{ck:+,.2f}")
...
with tab2:
    ...
    with c_chart1:
        st.subheader("📊 每月業績對比 (橘：實績 | 藍：目標)")
        c_long = calc_df.melt(id_vars=["月份"], value_vars=["實際營收 (TWD)", "業績目標 (TWD)"], var_name="類型", value_name="金額")
